...
export default function Sidebar({
  ...
  return (
    <>
      <aside className="min-h-screen z-30 fixed">
...
export function SidebarItem({ icon, text, active, alert, href }) {
  ...
  return (
    <Link
      to={href}
      className={`relative flex items-center py-2 px-3 my-1 font-medium rounded-lg cursor-pointer transition-colors group justify-center ${
        active
          ? "bg-gradient-to-tr from-amber-400 to-indigo-100 text-amber-500"
    ...
    >
      ...
      {alert && (
        <div
          className={`absolute right-2 w-2 h-2 rounded-lg bg-amber-500 ${
            expanded ? "" : "top-2"
          }`}
        />
      )}
      ...
      {!expanded && (
        <div
          className={`absolute left-full rounded-lg px-2 py-1 ml-6 bg-amber-300 text-gray-700 text-sm capitalize invisible opacity-20 -translate-x-3 transition-all group-hover:visible group-hover:opacity-100 group-hover:translate-x-0`}
        >
          {text}
        </div>
      )}
